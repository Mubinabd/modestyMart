...
import (
	...
	helper"github.com/Mubinabd/modestyMart/internal/pkg/help"
...
func (r *NotificationRepo) NotifyAll(req *pb.NotificationMessage) (*pb.Void, error) {
	...
	user_query := `select email, full_name from users where role = $1 and deleted_at = 0`
	...
	for rows.Next() {
		var user pb.UserRes
		err := rows.Scan(&user.Id, &user.Email, &user.FullName, &user.Username)
		if err != nil {
			return nil, err
		}
	...
	for _, user := range users {
		...
		err = helper.SendVerificationCode(helper.Params{
			...
			Password: password,
			To:       user.Email,
			Message:  fmt.Sprintf("Hi %s", user.Username),
			Code:     req.Message,
		})
...
func (r *NotificationRepo) DeleteNotificcation(id *pb.GetById) (*pb.Void, error) {
